...
public class StringUtils
{
	 ...
	 * address "matt@jivesoftware.com/Smack", "matt" would be returned. If
	 * no username is present in the address, the empty string will be
	 * returned.
	 * 
	 * @param XMPPAddress
	 *                  the XMPP address.
	 ...
	 * returned. If no server is present in the address, the empty string
	 * will be returned.
	 * 
	 * @param XMPPAddress
	 *                  the XMPP address.
	...
	/**
	 * Returns the resource portion of a XMPP address. For example, for
	 * the address "matt@jivesoftware.com/Smack", "Smack" would be
	 * returned. If no resource is present in the address, the empty
	...
	/**
	 * Escapes all necessary characters in the String so that it can be
	 * used in an XML doc.
	 * 
	 * @param string
	...
	public static String escapeForXML(String string)
	{
		...
		for (; i < len; i++)
		{
			...
			else if (ch == '&')
			{
				...
				// Do nothing if the string is of the
				// form &#235; (unicode value)
	...
	/**
	 * Hashes a String using the SHA-1 algorithm and returns the result as
	 * a String of hexadecimal numbers. This method is synchronized to
	 * avoid excessive MessageDigest object creation. If calling this
	 * method becomes a bottleneck in your code, you may wish to maintain
	 * a pool of MessageDigest objects instead of using this method.
	 * <p>
	 * A hash is a one-way function -- that is, given an input, an output
	 * is easily computed. However, given the output, the input is almost
	 * impossible to compute. This is useful for passwords since we can
	 * store the hash and a hacker will then have a very hard time
	 ...
	 * @param lineBreaks
	 *                  True if the encoding should contain line breaks
	 ...
	 * The Random class is not considered to be cryptographically secure,
	 * so only use these random Strings for low to medium security
	 * applications.
	 */
	private static Random randGen = new Random();

	/**
	 * Array of numbers and letters of mixed case. Numbers appear in the
	 * list twice so that there is a more equal chance that a number will
	 * be picked. We can use the array to get a random number or letter by
	 ...
	 * case) of the specified length. The method uses the Random class
	 * that is built-in to Java which is suitable for low to medium grade
	 * security uses. This means that the output is only pseudo random,
	 * i.e., each number is mathematically generated so is not truly
	 * random.
	 * <p>
	 * 
	 * The specified length must be at least one. If not, the method will
	 * return null.
	 * 
	 * @param length
	 *                  the desired length of the random String to return.
	 * @return a random String of numbers and letters of the specified
	 *         length.
	 */
	public static String randomString(int length)
	{
		...
		// Create a char buffer to put random letters and numbers
		// in.
	...
	/**
	 * Hashes a String using the specified algorithm and returns the
	 * result as a String of hexadecimal numbers. This method is
	 * synchronized to avoid excessive MessageDigest object creation. If
	 * calling this method becomes a bottleneck in your code, you may wish
	 * to maintain a pool of MessageDigest objects instead of using this
	 * method. <p/> A hash is a one-way function -- that is, given an
	 * input, an output is easily computed. However, given the output, the
	 * input is almost impossible to compute. This is useful for passwords
	 * since we can store the hash and a hacker will then have a very hard
	 * time determining the original password. <p/> In Jive, every time a
	 * user logs in, we simply take their plain text password, compute the
	 * hash, and compare the generated hash to the stored hash. Since it
	 * is almost impossible that two passwords will generate the same
	 * hash, we know if the user gave us the correct password or not. The
		...
		synchronized (algorithm.intern())
		{
			...
			if (digest == null)
			{
				...
				catch (NoSuchAlgorithmException nsae)
				{
					...
					// the " + algorithm + "
					// MessageDigest. " +
					// "Jive will be unable to
					// function normally.", nsae);
					return null;
				}
			}
			// Now, compute hash.
	 ...
	 * @return
	 */
	public static String lowerJIDUsername(String jid)
	{
		String username = parseName(jid);
		String server = parseServer(jid);
		String resource = parseResource(jid);
		
		if ((username == null || username.isEmpty()) || (server == null || server.isEmpty()))
		{
			return jid;
		}
		
		StringBuffer buf = new StringBuffer();
		buf.append(username.toLowerCase());
