package net.sf.mxlosgi.mxlosgixmppparserbundle.impl;
...
import net.sf.mxlosgi.mxlosgixmppbundle.PrivacyItem;
...
import net.sf.mxlosgi.mxlosgixmppparserbundle.UnknownPacketExtension;
...
public class XMPPParserImpl implements XMPPParser
{
	...
	private Map<String, ExtensionParser> extensionParsers = new ConcurrentHashMap<String, ExtensionParser>();
	
	/**
	 * 
	 */
	public XMPPParserImpl()
	{
	}
	...
	{
		extensionParsers.put(getKey(parser.getElementName(), parser.getNamespace()), parser);
	}

	@Override
	public ExtensionParser getExtensionParser(String elementName, String namespace)
	{
		return extensionParsers.get(getKey(elementName, namespace));
	}

	@Override
	public Collection<ExtensionParser> getExtensionParsers()
	{
		return Collections.unmodifiableCollection(extensionParsers.values());
	}

	@Override
	public void removeExtensionParser(ExtensionParser parser)
	{
		extensionParsers.remove(getKey(parser.getElementName(), parser.getNamespace()));
	}
	...
	private XMLStanza parseMessage(XmlPullParser parser) throws Exception
	{
		...
		while (!done)
		{
			...
			if (eventType == XmlPullParser.START_TAG)
			{
				...
				{
					ExtensionParser xparser = getExtensionParser(elementName, namespace);
					if (xparser != null)
					{
